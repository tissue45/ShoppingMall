...
import { User, Mail, Shield, Save, Loader, Building2, Phone, MapPin, Camera } from 'lucide-react';

const Profile = () => {
  const { user, updateProfile, loading, error, setError, isAdmin } = useAuth();
  ...
  useEffect(() => {
    const loadBrandData = async () => {
      ...
      try {
        ...
        console.error('브랜드 데이터 로드 중 오류:', error);
      } finally {
  ...
  const handleBrandUpdate = async (e) => {
    ...
    try {
      ...
      if (error) {
        console.error('브랜드 정보 업데이트 오류:', error);
        alert('브랜드 정보 업데이트에 실패했습니다.');
        return;
      }
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginBottom: '2rem' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <h3 style={{ margin: '0 0 0.5rem', color: '#333' }}>
            {brandSettings.name || user?.user_metadata?.name || '사용자'}
          </h3>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    style={{
                      ...
                      borderRadius: '6px',
                      resize: 'vertical',
                      backgroundColor: brandLoading ? '#f8f9fa' : 'white'
                    }}
                  />
                </div>
            ...
            <form onSubmit={handleBrandUpdate}>
              <div style={{ display: 'grid', gap: '1rem' }}>
                <div>
                  ...
                  <input
                    type="text"
                    value={brandSettings.businessNumber}
                    onChange={(e) => handleBrandChange('businessNumber', e.target.value)}
                    disabled={brandLoading}
                    style={{
                      width: '100%',
                      padding: '0.75rem',
                ...
                <div style={{ textAlign: 'right', marginTop: '1rem' }}>
                  <button
                    type="submit"
                    disabled={brandLoading || isSaving}
