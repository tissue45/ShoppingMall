...
// 실패한 브랜드들 (Cook&Joy는 사업자번호 변경)
const failedBrands = [
  { name: 'Cook&Joy', email: 'cooknjoy@cooknjoy.com', business_number: '999-99-99999', phone: '02-1234-5678' }, // 사업자번호 변경
...
async function registerFailedBrands() {
  console.log(`${failedBrands.length}개 실패한 브랜드 재등록을 시작합니다...\n`);
  
  const results = [];
  
  for (let i = 0; i < failedBrands.length; i++) {
    const result = await registerBrand(failedBrands[i]);
    results.push(result);
    
    // API 호출 간격을 더 길게 (Rate Limit 방지)
...
// 실행
registerFailedBrands().catch(console.error);
